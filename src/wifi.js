...
const Wifi = () => {
    const [distance, setDistance] = useState(30); // Initial distance value
    const [status, setStatus] = useState('0'); // Initial status value
    const [isConnected, setIsConnected] = useState(false);
    const [isFirebaseConnected, setIsFirebaseConnected] = useState(false);


    useEffect(() => {
        // Fetch distance from Firebase Realtime Database
        ...
        // Fetch status from Firebase Realtime Database
        ...
        return () => {
            // Firebase automatically handles cleanup of listeners when component unmounts
        };
    }, []);
            ...
            <div className="flex-grow flex flex-col justify-center items-center">
                ...
                <div className='mt-3 p-2 h-8 bg-gray-100 rounded-full flex items-center justify-center w-64'>
                    <div className='flex items-center'>
                        ...
                        <span className="text-sm text-blue-500">{isFirebaseConnected ? 'Connected' : 'Disconnected'}</span>
                        <FontAwesomeIcon
                            icon={faBluetoothB}
            ...
            <div className="flex items-center justify-center m-20">
                ...
                <div className="main2 p-4 rounded-lg shadow-md w-full max-w-screen-lg">
                    ...
                    <div className="text-center h-15">
                        <p className={`text-4xl font-bold -mt-10 ${status === '1' ? 'text-green-500' : 'text-blue-500'}`}>
                            {status === '1' ? 'UNLOCKED' : 'LOCKED'}
                        </p>
                    </div>
                    {/* Middle animation */}
                    <div className="mt-4 flex justify-center h-40">
                        <Lottie
                            animationData={status === '1' ? car : loading}
